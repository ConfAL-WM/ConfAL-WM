...
if os.environ.get("EVAC_DISABLE_XFORMERS", "").lower() in {"1", "true", "yes"}:
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, seq_length=None):
        ...
        def sdpa_no_xformers(q_, k_, v_, attn_mask_=None):
            q_ = rearrange(q_, '(b h) n d -> b h n d', h=h)
            k_ = rearrange(k_, '(b h) n d -> b h n d', h=h)
            v_ = rearrange(v_, '(b h) n d -> b h n d', h=h)
            if attn_mask_ is not None:
                attn_mask_ = rearrange(attn_mask_, '(b h) i j -> b h i j', h=h)
            out_ = F.scaled_dot_product_attention(
                q_, k_, v_, attn_mask=attn_mask_, dropout_p=0.0
            )
            return rearrange(out_, 'b h n d -> (b h) n d')
        ...
        if k is not None:
            ...
            if self.relative_position:
                ...
                attn_mask = None
                if exists(mask):
                    attn_mask = repeat(mask > 0.5, 'b i j -> (b h) i j', h=h)
                out = sdpa_no_xformers(q, k, v, attn_mask)
            del k
        ...
        if k_ip is not None:
            ...
            out_ip = sdpa_no_xformers(q, k_ip, v_ip)
            del k_ip
        ...
        ## for traj cross-attention
        if k_tp is not None:
            k_tp, v_tp = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (k_tp, v_tp))
            out_tp = sdpa_no_xformers(q, k_tp, v_tp)
            del k_tp
            out_tp = rearrange(out_tp, '(b h) n d -> b n (h d)', h=h)
        ...
        if k_vpre is not None:
            ...
            out_vpre = sdpa_no_xformers(q, k_vpre, v_vpre)
            del k_vpre
