...
if os.environ.get("EVAC_DISABLE_XFORMERS", "").lower() in {"1", "true", "yes"}:
    ...
    logpy.warning("EVAC_DISABLE_XFORMERS is set. Processing without xformers...")
else:
    try:
        import xformers
        import xformers.ops
...
def make_attn(in_channels, attn_type="vanilla", attn_kwargs=None):
    ...
    if attn_type == "vanilla":
        ...
        if not XFORMERS_IS_AVAILABLE:
            logpy.warning("xformers requested but unavailable/disabled; using vanilla attention.")
            return AttnBlock(in_channels)
        logpy.info(
        ...
        if not XFORMERS_IS_AVAILABLE:
            raise RuntimeError("memory-efficient-cross-attn requires xformers.")
        ...
        if not XFORMERS_IS_AVAILABLE:
            raise RuntimeError("spatial-temporal-xformer requires xformers.")
