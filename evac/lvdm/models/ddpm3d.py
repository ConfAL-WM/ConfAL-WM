...
from lvdm.data.traj_vis_statistics import ColorMapLeft, ColorMapRight, ColorListLeft, ColorListRight
...
class ACWMLatentDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True, unet_trainable_list=None,
                ...
                enable_c3_probe=False, c3_probe_config=None,
                patch_weighting_config=None,
        ...
        self.patch_weighting_config = patch_weighting_config or {}
        self.patch_weighting_enabled = bool(self.patch_weighting_config.get("enabled", False))
        ...
        if enable_c3_probe:
            ...
            probe_kwargs = c3_probe_config if c3_probe_config is not None else {}
            self.c3_probe = C3ConfidenceProbe(**probe_kwargs)
            print(f"[ACWMLatentDiffusion] C3ConfidenceProbe enabled. "
                  f"Params: {sum(p.numel() for p in self.c3_probe.parameters())/1e6:.2f}M")
    ...
    # ------------------------------------------------------------------
    # C3 Probe integration
    # ------------------------------------------------------------------

    def apply_model(self, x_noisy, t, cond, return_probe_feat=False, probe_tau=None, **kwargs):
        """
        Overrides LatentDiffusion.apply_model to optionally return C3 probe
        features alongside the UNet output.

        Args:
            return_probe_feat: if True AND self.c3_probe is not None,
                returns (x_recon, probe_logits) instead of just x_recon.
                probe_logits: [B, T, 10, 16] — raw (pre-sigmoid) confidence.

        Normal call (return_probe_feat=False) is identical to the parent.
        """
        if isinstance(cond, dict):
            pass
        else:
        ...
        use_probe = return_probe_feat and (self.c3_probe is not None)

        result = self.model(x_noisy, t, return_probe_feat=use_probe, **cond, **kwargs)

        if use_probe:
            x_recon, (h_mid, h_dec, emb) = result
            # x_noisy shape: [(B×V), C, T, h_lat, w_lat]
            T = x_noisy.shape[2]
            from evac.lvdm.modules.c3_probe_decoder import C3ConfidenceProbeDecoder
            if isinstance(self.c3_probe, C3ConfidenceProbeDecoder):
                probe_logits = self.c3_probe(h_dec, T=T)
            else:
                probe_logits = self.c3_probe(
                    h_dec=h_dec,
                    T=T,
                    h_mid=h_mid,
                    emb=emb,
            ...
            return x_recon, probe_logits

        # normal path: identical to parent
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        ...
        if "confidence_map" in batch:
            # Dense C3 confidence is a guide only. It is detached in
            # _build_patch_loss_weight and never receives gradients.
            kwargs.update({"confidence_map": batch["confidence_map"]})
    ...
    def _build_patch_loss_weight(self, confidence_map, loss_simple_map):
        if not self.patch_weighting_enabled or confidence_map is None:
            return None, {}

        cfg = self.patch_weighting_config
        device = loss_simple_map.device
        dtype = loss_simple_map.dtype
        b, _, t, h, w = loss_simple_map.shape

        conf = confidence_map.detach().to(device=device, dtype=torch.float32)
        if conf.ndim == 3:
            conf = conf.unsqueeze(0).unsqueeze(1)  # [1, 1, T, H, W]
        elif conf.ndim == 4:
            conf = conf.unsqueeze(1)               # [B, 1, T, H, W]
        elif conf.ndim == 5:
            if conf.shape[1] != 1:
                conf = conf.mean(dim=1, keepdim=True)
        else:
            raise ValueError(f"confidence_map must be [T,H,W], [B,T,H,W], or [B,1,T,H,W], got {tuple(conf.shape)}")

        if conf.shape[0] != b:
            if conf.shape[0] == 1:
                conf = conf.repeat(b, 1, 1, 1, 1)
            elif b % conf.shape[0] == 0:
                conf = conf.repeat_interleave(b // conf.shape[0], dim=0)
            else:
                raise ValueError(f"confidence_map batch {conf.shape[0]} cannot align with loss batch {b}")

        confidence_format = str(cfg.get("confidence_format", "probability"))
        if confidence_format == "logits":
            conf = torch.sigmoid(conf)
        elif confidence_format != "probability":
            raise ValueError(f"Unsupported patch confidence_format={confidence_format!r}")

        conf = torch.nan_to_num(conf, nan=1.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
        risk = 1.0 - conf
        if risk.shape[-3:] != (t, h, w):
            risk = F.interpolate(risk, size=(t, h, w), mode="trilinear", align_corners=False)

        q_low = float(cfg.get("risk_quantile_low", 0.05))
        q_high = float(cfg.get("risk_quantile_high", 0.95))
        q_low = min(max(q_low, 0.0), 1.0)
        q_high = min(max(q_high, q_low + 1e-4), 1.0)
        flat = risk.flatten(1)
        lo = torch.quantile(flat, q_low, dim=1).view(b, 1, 1, 1, 1)
        hi = torch.quantile(flat, q_high, dim=1).view(b, 1, 1, 1, 1)
        risk_norm = ((risk - lo) / (hi - lo + 1e-6)).clamp(0.0, 1.0)

        gamma = float(cfg.get("risk_gamma", 1.0))
        if gamma != 1.0:
            risk_norm = risk_norm.clamp_min(1e-6).pow(gamma)

        target_lambda = float(cfg.get("lambda_conf", cfg.get("lambda", 1.0)))
        warmup_steps = int(cfg.get("weight_warmup_steps", 0))
        if warmup_steps > 0:
            warmup = min(1.0, float(getattr(self, "global_step", 0)) / float(max(1, warmup_steps)))
        else:
            warmup = 1.0
        lambda_eff = target_lambda * warmup

        weight = 1.0 + lambda_eff * risk_norm
        if bool(cfg.get("preserve_mean", True)):
            weight = weight / weight.flatten(1).mean(dim=1).view(b, 1, 1, 1, 1).clamp_min(1e-6)

        w_min = float(cfg.get("weight_clip_min", 0.5))
        w_max = float(cfg.get("weight_clip_max", 3.0))
        weight = weight.clamp(w_min, w_max)
        ...
        stats = {
            ...
            "risk_mean": float(risk.mean().detach().cpu()),
            "risk_norm_mean": float(risk_norm.mean().detach().cpu()),
        }
        return weight.to(dtype=dtype), stats

    def p_losses(self, x_start, cond, t, noise=None, traj_aug_mask=None, **kwargs):
        confidence_map = kwargs.pop("confidence_map", None)
        ...
        patch_weight, patch_stats = self._build_patch_loss_weight(confidence_map, loss_simple_map)
        if patch_weight is not None:
            effective_weight = patch_weight
            if traj_aug_mask is not None:
                traj_mask = 1.0 - traj_aug_mask[:, :, -self.chunk:].to(device=loss_simple_map.device, dtype=loss_simple_map.dtype)
                effective_weight = effective_weight * traj_mask
            denom = effective_weight.expand_as(loss_simple_map).sum(dim=(1, 2, 3, 4)).clamp_min(1e-6)
            loss_simple = (loss_simple_map * effective_weight).sum(dim=(1, 2, 3, 4)) / denom
            loss_dict.update({
                f'{prefix}/patch_weight_mean': patch_stats["weight_mean"],
                f'{prefix}/patch_weight_min': patch_stats["weight_min"],
                f'{prefix}/patch_weight_max': patch_stats["weight_max"],
                f'{prefix}/patch_risk_mean': patch_stats["risk_mean"],
                f'{prefix}/patch_risk_norm_mean': patch_stats["risk_norm_mean"],
                f'{prefix}/patch_lambda_eff': patch_stats["lambda_eff"],
            })
        elif traj_aug_mask is not None:
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, causal=False, chunk=4, cat_mask=False, sparse=False, inference=False, ddim_dtype=None, **kwargs):
        ...
        if ddim:
            # Cache DDIMSampler to avoid re-allocating schedule buffers (prevents OOM)
            cache_key = (self.ddim_num_chunk, ddim_dtype)
            if getattr(self, '_ddim_sampler_cache_key', None) != cache_key:
                self._ddim_sampler = DDIMSampler(self, num_chunk=self.ddim_num_chunk, dtype=ddim_dtype)
                self._ddim_sampler_cache_key = cache_key
    ...
    def inference(
        ...
        **kwargs
        ...
        w2c_list = w2c_list[:, :action.shape[0], ...]
        c2w_list = c2w_list[:, :action.shape[0], ...]
        n_valid = action.shape[0] - n_previous
        ...
        all_trajs = None
        all_c2ws_list = None
        return_latents = bool(kwargs.pop("return_latents", False))
        ...
        if return_latents:
            return x_samples_video, traj_samples_video, all_samples
        return x_samples_video, traj_samples_video


    @torch.no_grad()
    def get_traj(self, sample_size, pose, w2c, c2w, intrinsic, radius=None):
        """
        ...
        cvt_matrix = torch.eye(4, dtype=torch.float32, device=pose.device).view(1,1,4,4)
        ...
        all_img_list = rearrange(torch.tensor(all_img_list, dtype=torch.float32).to(device=pose.device), "v t h w c -> c v t h w")
